...
for ec = 1:1:length(erp.eventCodes)-1
    if (erp.eventCodes(ec) >220 && erp.eventCodes(ec) < 225)
...
choice2_erp = struct();
reject_choice2_erp = struct();
choice2_erp_ave = struct();
...
for condition = 1:1:length(conditions_base)
    ...
    %choice2_erp.(trial_name)=[squeeze(erp.trialNum.(condition_name))];

    
    %reject_choice2_erp.(trial_name)=[squeeze(erp.trialNum.(condition_name))];
    ...
    for channel = 1:1:length(channels)
        ...
        choice2_erp.(lat_fieldname) = [squeeze(erp.trial.(l_condition_name)(:,channel,:));squeeze(erp.trial.(r_condition_name)(:,channel_r_index(channel),:))];
        choice2_erp.(mid_fieldname) = [squeeze(erp.trial.(u_condition_name)(:,channel,:));squeeze(erp.trial.(b_condition_name)(:,channel,:))];
        choice2_erp.(lat_fieldname_hilb) = [squeeze(erp.trial_hilb.(l_condition_name)(:,channel,:));squeeze(erp.trial_hilb.(r_condition_name)(:,channel_r_index(channel),:))];
        choice2_erp.(mid_fieldname_hilb) = [squeeze(erp.trial_hilb.(u_condition_name)(:,channel,:));squeeze(erp.trial_hilb.(b_condition_name)(:,channel,:))];

        reject_choice2_erp.(lat_fieldname) = [squeeze(erp.reject_trial.(l_condition_name)(:,channel,:));squeeze(erp.reject_trial.(r_condition_name)(:,channel_r_index(channel),:))];
        reject_choice2_erp.(mid_fieldname) = [squeeze(erp.reject_trial.(u_condition_name)(:,channel,:));squeeze(erp.reject_trial.(b_condition_name)(:,channel,:))];
        
        choice2_erp_ave.(lat_fieldname) = nanmean(choice2_erp.(lat_fieldname)(:,:),1);
        choice2_erp_ave.(mid_fieldname) = nanmean(choice2_erp.(mid_fieldname)(:,:),1);
...
%Now for LRP!
conditions_lrp = ['Target1';'Target2';'Target3'];
for condition = 1:1:size(conditions_lrp,1)
    condition_name = char(conditions_lrp(condition,:));
    for channel = 1:1:length(channels)
        channel_name = char(channels(channel,:));
        u_name = ['U',condition_name];
        l_name = ['L',condition_name];
        r_name = ['B',condition_name];
        b_name = ['R',condition_name];
        
        outname = [condition_name,'_',channel_name];
        choice2_lrp.(outname)= [squeeze(erp.trial.(u_name)(:,channel,:));squeeze(erp.trial.(l_condition_name)(:,channel,:));squeeze(erp.trial.(b_condition_name)(:,channel_r_index(channel),:));squeeze(erp.trial.(r_condition_name)(:,channel_r_index(channel),:))];
        
        choice2_lrp_ave.(outname)=nanmean(choice2_lrp.(outname)(:,:),1);
    end;
end;
...
for condition = 1:1:length(conditions_base)
    ...
    choice2_art.(lat_art_free_name)= (art_free_counter((condition-1)*4+1)+art_free_counter((condition-1)*4+4))/(art_free_counter((condition-1)*4+1)+art_free_counter((condition-1)*4+4)+art_counter((condition-1)*4+1)+art_counter((condition-1)*4+4));
    choice2_art.(lat_blink_name)= (blink_counter((condition-1)*4+1)+blink_counter((condition-1)*4+4));
    choice2_art.(lat_eMove_name)= (eMove_counter((condition-1)*4+1)+eMove_counter((condition-1)*4+4));
    choice2_art.(lat_blocking_name)= (blocking_counter((condition-1)*4+1)+blocking_counter((condition-1)*4+4));
    
    choice2_art.(mid_art_free_name)= (art_free_counter((condition-1)*4+2)+art_free_counter((condition-1)*4+3))/(art_free_counter((condition-1)*4+2)+art_free_counter((condition-1)*4+3)+art_counter((condition-1)*4+2)+art_counter((condition-1)*4+3));
    choice2_art.(mid_blink_name)= (blink_counter((condition-1)*4+2)+blink_counter((condition-1)*4+3));
    choice2_art.(mid_eMove_name)= (eMove_counter((condition-1)*4+2)+eMove_counter((condition-1)*4+3));
    choice2_art.(mid_blocking_name)= (blocking_counter((condition-1)*4+2)+blocking_counter((condition-1)*4+3));
    
end;

%save('erp.mat','erp');
save('choice2_erp.mat','choice2_erp')
%save('reject_enc_erp.mat','reject_enc_erp')
save('choice2_erp_ave.mat','choice2_erp_ave');
save('choice2_art.mat','choice2_art');

save('choice2_lrp.mat','choice2_lrp')
save('choice2_lrp_ave.mat','choice2_lrp_ave')
