...
for condition = 1:1:size(conditions_lrp,1)
    ...
    for channel = 1:1:length(channels)
        ...
        choice1_lrp.(outname)= [squeeze(erp.trial.(u_name)(:,channel,:));squeeze(erp.trial.(l_condition_name)(:,channel,:));squeeze(erp.trial.(b_condition_name)(:,channel_r_index(channel),:));squeeze(erp.trial.(r_condition_name)(:,channel_r_index(channel),:))];
        
        choice1_lrp_ave.(outname)=nanmean(choice1_lrp.(outname)(:,:),1);
    end;
end;
...
save('choice1_lrp.mat','choice1_lrp')
save('choice1_lrp_ave.mat','choice1_lrp_ave')
